...
namespace scts {
	...
	struct basic_writer {
		...
		static scts::out_stream& write(const O& object, scts::out_stream& stream, const Names& names, std::size_t name_index) {
			Formatter::write_single(Member::get(object), stream, names.at(name_index), true);
			return stream;
		}

		template <typename Member, typename Second, typename... Rest>
		static scts::out_stream& write(const O& object, scts::out_stream& stream, const Names& names, std::size_t name_index) {
			Formatter::write_single(Member::get(object), stream, names.at(name_index), false);
	...
	struct basic_reader {
		...
		static O& read(O& object, scts::in_stream& stream) {
			Formatter::read_single(Member::get(object), stream);
			return object;
		}
